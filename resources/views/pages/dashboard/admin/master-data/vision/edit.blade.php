...
@section('content')
            ...
            <div class="card my-0">
                <div
                    ...
                    <div class="d-flex flex-column">
                        ...
                        <p class="p-0 m-0 fw-medium text-muted">Update this vision.</p>
                    </div>
    ...
    <div class="row">
        <div class="col">
            <div class="card my-0">
                <div class="card-body">
                    <form action="{{ route('dashboard.admin.master-data.visions.update', $vision->id) }}" autocomplete="off"
                        method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-floating mb-3">
                            <select name="order" id="floatingInputOrder"
                                ...
                                <option value="" disabled {{ old('order', $vision->order) ? '' : 'selected' }}>Select
                                    Order</option>
                                @for ($i = 1; $i <= $allowedMaxOrder; $i++)
                                    <option value="{{ $i }}"
                                        {{ old('order', $vision->order) == $i ? 'selected' : '' }}>
                                        {{ $i }}
                                    </option>
                                @endfor
                            </select>
