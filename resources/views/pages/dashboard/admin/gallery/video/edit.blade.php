...
@section('content')
            ...
            <div class="card my-0">
                <div
                    ...
                    <div class="d-flex flex-column">
                        <h3 class="p-0 m-0 mb-1 fw-semibold">Edit Video</h3>
                        <p class="p-0 m-0 fw-medium text-muted">Update this video.</p>
                    </div>
    ...
    <div class="row">
        <div class="col">
            <div class="card my-0">
                <div class="card-body">
                    <form action="{{ route('dashboard.admin.gallery.videos.update', $video->id) }}" autocomplete="off"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" name="video_file" id="videoInput" class="form-control form-control-sm @error('video_file') is-invalid @enderror" accept="video/*">
                            <div class="mt-2 small text-muted">
                                Selecting a new file will replace the preview video.
                            </div>
                            <button type="button" id="removeVideoBtn" class="btn btn-sm btn-danger mt-2 d-none">
                                <i class="ti ti-trash me-1"></i> Cancel Selected Video
                            </button>
                            @error('video_file')
                        ...
                        <div class="form-floating mb-3">
                            <select name="group_id" id="floatingInputOrder" class="form-select @error('group_id') is-invalid @enderror">
                                ...
                                @foreach ($groups as $group)
                                    <option value="{{ $group->id }}" {{ old('group_id', $video->group_id) == $group->id ? 'selected' : '' }}>
                                        {{ $group->title }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="d-flex justify-content-end">
                            <button type="submit" class="btn btn-primary d-flex align-items-center gap-2 justify-content-center px-4 rounded-pill">
                                <i class="ti ti-device-floppy me-1"></i> Update Video
                            </button>
                        </div>
                    </form>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const originalVideo = "{{ $video->file_url }}";
